...
abstract class BaseKonfiguration implements Konfiguration {

    @Getter(AccessLevel.PACKAGE)
    private final KonfigObserversManager konfigObserversManager
            = new KonfigObserversManager();

    protected abstract <T> KonfigV<T> get(KonfigKey konfigKey, boolean mustExist);

    protected abstract <T> T v(KonfigKey key);

    protected abstract <T> T v(KonfigKey key, T def);
    ...
    private <T> KonfigV<T> get(@NonNull String key, @NonNull Class<?> dt, Class<?> el) {
        ...
        return get(new KonfigKey(key, dt, el), true);
    }

    private <T> KonfigV<T> getD(@NonNull String key, @NonNull Class<?> dt, Class<?> el) {

        return get(new KonfigKey(key, dt, el), false);
    }
